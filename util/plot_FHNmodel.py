...
data = np.loadtxt('FHNmodel_u.000.txt', dtype=np.double)
nt = np.shape(data)[0]
...
if (nprocs > 1):
    for isub in range(1,nprocs):
        data = np.loadtxt('FHNmodel_u.' + repr(isub).zfill(3) + '.txt', dtype=np.double)
...
for tstep in range(nt):
    ...
    pname = 'FHNmodel_surf_u.' + repr(tstep).zfill(3) + '.png'
    tstr  = repr(tstep)
    nxstr = repr(nx)
    nystr = repr(ny)
    ...
    ax.view_init(90,90)
    title('Flat: u(x,y) at output ' + tstr + ', mesh = ' + nxstr + 'x' + nystr)
    savefig(pname)
    plt.close()


os.system("convert -delay 30 -loop 0 FHNmodel_surf_u.*.png " + "FHNmodel_flat_beta_14.gif")
os.system("rm FHNmodel_surf_u.*.png") # clean up files
os.system("animate FHNmodel_flat_beta_14.gif") # play animates gif
